...
- 對應 chat 工具：project_info(category=decisions / standards / changelog)`);
  ...
  // ── Load EM prompt from file (with fallback to inline default) ──
  const emPromptPath = join(rootDir, '.paaw', 'auto-dispatch', 'em-prompt.md');
  let emPromptTemplate;
  try {
    emPromptTemplate = await readFile(emPromptPath, 'utf-8');
  } catch {
    // Fallback: minimal inline prompt
    emPromptTemplate = `你是 AI Coding Team 的 Engineering Manager (陳哲宇 Ethan)。{{exclusionText}}
...
## 輸出格式（嚴格 JSON array）
\`\`\`json
[{"agent":"developer","task":"...","priority":"high","reason":"..."}]
\`\`\``;
  }

  // ── Replace placeholders with dynamic values ──
  const EM_PROMPT = emPromptTemplate
    .replace(/\{\{exclusionText\}\}/g, exclusionText)
    .replace(/\{\{agentListText\}\}/g, agentListText)
    .replace(/\{\{strategyDesc\}\}/g, strategyDesc)
    .replace(/\{\{phaseConstraints\}\}/g, phaseConstraints)
    .replace(/\{\{scopeText\}\}/g, scopeText)
    .replace(/\{\{minSubs\}\}/g, String(Math.min(maxSubs, 5)))
    .replace(/\{\{maxSubs\}\}/g, String(maxSubs))
    .replace(/\{\{minQuality\}\}/g, String(Math.min(Math.floor(maxSubs/2), 5)))
    .replace(/\{\{maxQuality\}\}/g, String(Math.min(maxSubs-2, 8)))
    .replace(/\{\{estimateLine\}\}/g, requireEstimate ? `- 每項必須附預估 effort（${defaultEffort} 為預設）` : '')
    .replace(/\{\{reportFormat\}\}/g, reportFormat)
    .replace(/\{\{reportFormatDesc\}\}/g, reportFormat === 'executive' ? '（簡潔決策導向）' : reportFormat === 'detailed' ? '（完整細節）' : '（摘要）');
