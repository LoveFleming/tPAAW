 ...
 *
 * 支援兩種 runner：
 *   1. api — 通用 HTTP 呼叫，tool.json 裡定義 URL/headers/body 模板
 *   2. script — 自訂 handler.mjs
 *
 * 目錄結構（支援兩種）：
...
// ── Scan tool defs from provider dir (supports both patterns) ──
...
// ── Register a tool from toolDef ──

async function registerTool(toolDef, config, providerDir, sourceLabel) {
...
export async function initProviderTool(providerId) {
  ...
  for (const toolDef of toolDefs) {
    try {
      const ok = await registerTool(toolDef, config, providerDir, sourceLabel);
...
function replaceTemplate(template, args, config) {
  ...
  return template.replace(/\{\{([^}]+)\}\}/g, (_, key) => {
    if (key.startsWith("@")) {
      ...
      const gen = GENERATORS[parts[0]];
      if (gen) return gen(parts[1] || "");
      return `{{${key}}}`;
    }
...
// ── List all available tool names (for workflow UI) ──
