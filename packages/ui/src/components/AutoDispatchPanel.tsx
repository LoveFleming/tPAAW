  ...
  return (
    <div className="flex h-full" style={{ background: tk.bg }}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* ── Health Fix Tasks ── */}
        <HealthFixTasks rootPath={rootPath} theme={tk} />
      </div>
    </div>
  );
}

// ── Health Fix Tasks: 顯示派工修復的 task + sub-task 狀態 + 中斷按鈕 ──
function HealthFixTasks({ rootPath, theme: tk }: { rootPath?: string; theme: any }) {
  const [healthTasks, setHealthTasks] = useState<any[]>([]);

  const loadHealthTasks = useCallback(async () => {
    if (!rootPath) return;
    try {
      const r = await fetch(`${API_BASE}/api/coding-tasks?path=${encodeURIComponent(rootPath)}&type=health`);
      const data = await r.json();
      const parents = (data.tasks || []).filter((t: any) => t.type === "health" && !t.parentId);
      const enriched = await Promise.all(parents.slice(0, 10).map(async (p: any) => {
        try {
          const r2 = await fetch(`${API_BASE}/api/coding-tasks?path=${encodeURIComponent(rootPath)}&parentId=${p.id}`);
          const d2 = await r2.json();
          return { ...p, _subTasks: d2.tasks || [] };
        } catch {
          return { ...p, _subTasks: [] };
        }
      }));
      setHealthTasks(enriched);
    } catch {}
  }, [rootPath]);

  useEffect(() => { loadHealthTasks(); }, [loadHealthTasks]);
  useEffect(() => {
    const hasRunning = healthTasks.some(t => t.status === "open" || t.status === "in-progress");
    if (!hasRunning) return;
    const iv = setInterval(loadHealthTasks, 10000);
    return () => clearInterval(iv);
  }, [healthTasks, loadHealthTasks]);

  const handleAbort = async (taskId: string) => {
    if (!rootPath) return;
    try {
      await fetch(`${API_BASE}/api/coding-tasks/${encodeURIComponent(taskId)}?path=${encodeURIComponent(rootPath)}`, { method: "DELETE" });
      loadHealthTasks();
    } catch {}
  };

  if (healthTasks.length === 0) return null;

  return (
    <div className="mt-4 pt-3 border-t" style={{ borderColor: tk.borderLight }}>
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-xs font-bold flex items-center gap-1" style={{ color: tk.text }}>
          <span>🏥</span> 派工修復
        </h3>
        <button onClick={loadHealthTasks} className="text-[10px] px-1.5 py-0.5 rounded" style={{ color: tk.text, opacity: 0.5 }}>↻</button>
      </div>
      <div className="space-y-2">
        {healthTasks.map(task => {
          const subs = task._subTasks || [];
          const doneCount = subs.filter((s: any) => s.status === "resolved" || s.pipeline?.implement?.status === "done").length;
          const isRunning = task.status === "open" || task.status === "in-progress";
          return (
            <div key={task.id} className="rounded-lg border overflow-hidden" style={{ borderColor: tk.borderLight }}>
              <div className="flex items-center gap-2 px-2.5 py-1.5" style={{ backgroundColor: tk.bgMuted }}>
                <span className="text-sm">{task.status === "resolved" ? "✅" : isRunning ? "🔄" : "⏳"}</span>
                <span className="text-[11px] font-bold flex-1 truncate" style={{ color: tk.text }}>{task.title}</span>
                <span className="text-[10px]" style={{ color: tk.text, opacity: 0.5 }}>{doneCount}/{subs.length}</span>
                {isRunning && (
                  <button
                    onClick={() => handleAbort(task.id)}
                    className="text-[10px] px-1.5 py-0.5 rounded bg-red-100 text-red-600 hover:bg-red-200 font-bold transition-colors shrink-0"
                    title="中斷"
                  >⏹ 中斷</button>
                )}
              </div>
              {subs.length > 0 && (
                <div className="px-2.5 py-1 space-y-0.5">
                  {subs.map((st: any) => {
                    const stDone = st.status === "resolved" || st.pipeline?.implement?.status === "done";
                    const stRunning = st.status === "in-progress" || st.status === "open";
                    return (
                      <div key={st.id} className="flex items-center gap-1.5 text-[10px]">
                        <span className="w-3 text-center">{stDone ? "✅" : stRunning ? "🔄" : "⏳"}</span>
                        <span className="font-medium" style={{ color: tk.accent }}>{st.assignee || "?"}</span>
                        <span className="flex-1 truncate" style={{ color: tk.text, opacity: 0.7 }}>{st.title || st.description}</span>
                        {stRunning && (
                          <button
                            onClick={() => handleAbort(st.id)}
                            className="px-1 py-0 rounded text-red-500 hover:bg-red-50 transition-colors"
                            title="中斷此 sub-task"
                          >⏹</button>
                        )}
                      </div>
