...
export default function WorkflowExec() {
  ...
  useEffect(() => {
    fetch(`${API}/api/paaw/workflows`).then(r => r.json()).then((l: WorkflowDef[]) => { setWorkflows(l); }).catch(() => {});
  }, []);
  ...
  const runWorkflow = useCallback(async () => {
    ...
    const sorted = topoSort(currentWf.nodes, currentWf.edges);
    ...
    for (const node of sorted) {
      ...
      try {
        ...
        if (result.error) { log[log.length - 1] = { nodeId: node.id, nodeName: node.name, status: "error", error: result.error, durationMs: dur }; break; }
      ...
      } catch (err: any) { log[log.length - 1] = { nodeId: node.id, nodeName: node.name, status: "error", error: err.message, durationMs: Date.now() - start }; break; }
      setExecLog([...log]);
    }
    setIsRunning(false);
    if (lastId) setSelectedNodeId(lastId);
    showToast(errorCount > 0 || log.some(l => l.status === "error") ? "❌ 執行失敗" : "✅ 執行完成");
  }, [currentWf, workflowInput, errorCount]);
    ...
    <div className="flex h-full w-full relative bg-stone-50">
      ...
      <div className="w-56 border-r border-stone-200 bg-white flex flex-col">
        <div className="p-3 border-b border-stone-200">
          ...
          <div className="text-[10px] text-stone-400 mt-0.5">選擇 Workflow → 輸入 → 執行</div>
        </div>
          ...
          {workflows.map(wf => {
            ...
            return (
              <button key={wf.id} onClick={() => selectWf(wf.id)} className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors ${currentWf?.id === wf.id ? "bg-violet-100 text-violet-800" : "hover:bg-stone-50 text-stone-600"}`}>
                ...
                <div className="text-[10px] text-stone-400 mt-0.5 ml-6">{nodeCount} 個積木</div>
              </button>
            );
          })}
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {/* Flow visualization (simple linear view) */}
        {currentWf && currentWf.nodes.length > 0 && (
          <div className="px-6 py-3 border-b border-stone-100 bg-white">
            <div className="flex items-center gap-1 overflow-x-auto pb-1">
              {topoSort(currentWf.nodes, currentWf.edges).map((node, i) => {
                const logEntry = execLog.find(e => e.nodeId === node.id);
                const status = logEntry?.status || "pending";
                const statusStyle = status === "running" ? "border-amber-400 bg-amber-50" : status === "success" ? "border-emerald-400 bg-emerald-50" : status === "error" ? "border-red-400 bg-red-50" : "border-stone-200 bg-white";
                const statusIcon = status === "running" ? "⏳" : status === "success" ? "✅" : status === "error" ? "❌" : "⬜";
                return (
                  <div key={node.id} className="flex items-center">
                    <button onClick={() => setSelectedNodeId(node.id)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border-2 text-xs font-medium transition-colors ${statusStyle} ${selectedNodeId === node.id ? "ring-2 ring-violet-300" : ""}`}>
                      <span>{statusIcon}</span>
                      <span className="text-stone-700">{node.name}</span>
                      <span className="text-stone-400">{node.skillId}</span>
                    </button>
                    {i < topoSort(currentWf.nodes, currentWf.edges).length - 1 && <span className="text-stone-300 mx-1">→</span>}
                  </div>
                );
              })}
            </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {!currentWf && (
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-stone-400">
                ...
                <div className="text-sm">從左側選擇一個 Workflow 開始</div>
              </div>
            </div>
          )}
          {currentWf && execLog.length === 0 && !isRunning && (
            <div className="flex items-center justify-center h-full">
              <div className="text-center text-stone-400">
                <div className="text-4xl mb-3">▶</div>
                <div className="text-sm">輸入資料，按 Run 執行 Workflow</div>
                <div className="text-xs mt-1">{currentWf.nodes.length} 個積木待執行</div>
              </div>
            </div>
          )}
            ...
            <div className="p-6 space-y-3">
              ...
              {execLog.map((entry, i) => (
                <div key={i} className={`flex items-center gap-3 px-4 py-3 rounded-lg border ${entry.status === "running" ? "border-amber-200 bg-amber-50" : entry.status === "success" ? "border-emerald-200 bg-emerald-50" : "border-red-200 bg-red-50"}`}>
                  <span className="text-sm">{entry.status === "running" ? "⏳" : entry.status === "success" ? "✅" : "❌"}</span>
          ...
          {!isRunning && execLog.length > 0 && (
            <div className="p-6 space-y-4">
              ...
              {/* Selected result */}
              {selectedLogEntry?.output && <ResultCards output={selectedLogEntry.output} />}
